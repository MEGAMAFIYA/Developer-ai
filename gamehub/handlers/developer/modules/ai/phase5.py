...
from handlers.developer.modules.ai.callbacks import (
    ...
    AI_CODE, AI_BUILDER, AI_GAMEPLAY,
    AI_DESIGN,
    AI_IMAGE, AI_ASSETS, AI_PREVIEW,
    AI_TEST,
    # Phase 5 sub-actions
    AI_CODE_SAVE,
    AI_BUILDER_SAVE, AI_BUILDER_DISCARD,
    AI_GAMEPLAY_LIST, AI_GAMEPLAY_SAVE_OK,
    AI_GAMEPLAY_DISCARD,
    AI_DESIGN_LIST, AI_DESIGN_SAVE_OK,
    AI_DESIGN_DISCARD,
    AI_IMAGE_SAVE, AI_IMAGE_DISCARD,
    AI_ASSETS_LIST, AI_ASSETS_UPLOAD,
    AI_ASSETS_DEL_OK, AI_ASSETS_DEL_NO,
    AI_PREVIEW_LIST,
    AI_TEST_CODE, AI_TEST_FILE,
    # Phase 3 — reused by Code Assistant keyboard
    AI_WRITE_CODE, AI_EDIT_CODE,
    AI_ANALYZE_CODE, AI_FIND_BUG, AI_FIX_BUG,
)

from handlers.developer.modules.ai.menu import (
    ai_back_keyboard, ai_menu_keyboard,
    AI_MENU_TEXT,
)

from handlers.developer.modules.ai.states import (
    AIBuilderStates, AIGameplayStates,
    AIDesignStates,
    AIAssetStates, AICodeSaveStates,
    AITestStates,
    AIAssetManagerFSM,
)
...
# ── Paths ────────────────────────────────────
# Must match the deployed checkout path
# (see services/github_service.py
# _DEPLOY_WEBAPP_ROOT) — Render's
# Dockerfile copies gamehub/ into the image,
# so the real repository path is
# gamehub/webapp/games, not the repo-root
# webapp/games tree.
_GAMES_DIR = "gamehub/webapp/games"
_TG_MAX = 4096

# Dynamic callback prefixes (matched with
# startswith in filters)
...
# Asset file extensions managed by Assets
# Manager
_ASSET_EXTS = {
    ".png", ".jpg", ".jpeg", ".webp",
    ".svg", ".gif",
...
async def _game_list_kb(
    prefix: str
) -> tuple[str, InlineKeyboardMarkup]:
    """Return (prompt_text, keyboard)
    showing all games as selection buttons."""
    ...
    return (
        f"🎮 O'yinni tanlang ({len(games)} ta):",
...
async def _show_code_result(
    ...
    if truncated:
        header += (
            f"\n<i>({len(full_code):,} belgi — "
            f"birinchi 2400 ko'rsatilmoqda)</i>"
        )
...
def _code_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(
                text="📝 Kod yozdirish",
                callback_data=AI_WRITE_CODE
            ),
            InlineKeyboardButton(
                text="✏️ Kodni tahrirlash",
                callback_data=AI_EDIT_CODE
            ),
        ],
        [
            InlineKeyboardButton(
                text="🔍 Kodni tahlil",
                callback_data=AI_ANALYZE_CODE
            ),
            InlineKeyboardButton(
                text="🧠 Bug topish",
                callback_data=AI_FIND_BUG
            ),
        ],
        [
            InlineKeyboardButton(
                text="❌ Xatoni tuzatish",
                callback_data=AI_FIX_BUG
            ),
            InlineKeyboardButton(
                text="💾 Faylga saqlash",
                callback_data=AI_CODE_SAVE
            ),
        ],
        [
            InlineKeyboardButton(
...
async def cb_ai_code(q: CallbackQuery) -> None:
    ...
    await q.message.edit_text(
        ...
        "HTML5 o'yinlar uchun kod yozish, "
        "tahrirlash, tahlil va saqlash.\n\n"
...
async def msg_code_save_receive(
    m: Message,
    state: FSMContext
) -> None:
    ...
    await m.answer(
        ...
        "Fayl nomini kiriting (masalan: "
        "<code>mening_oyinim.html</code>).\n"
...
async def msg_code_save_filename(
    m: Message,
    state: FSMContext
) -> None:
    if not await _guard_msg(m):
    ...
    if not result.ok:
        ...
        await sent.edit_text(
            f"❌ <b>Xato</b>\n\n"
            f"<code>{_esc(result.error or 'Noma`lum xato')}</code>",
            reply_markup=ai_back_keyboard(),
            parse_mode="HTML",
        )
        return
...
async def cb_builder_save_start(
    q: CallbackQuery,
    state: FSMContext
) -> None:
    if not await _guard(q):
...
async def msg_builder_filename(
    m: Message,
    state: FSMContext
) -> None:
    if not await _guard_msg(m):
    ...
    try:
        size = await _save_game_file(
            filename,
            game_code,
            f"AI game builder: {filename}",
        )

        slug = re.sub(
            r"[^a-z0-9_]",
            "_",
            filename.replace(".html", "").lower()
        )
...
@router.callback_query(
    lambda c: c.data in (AI_GAMEPLAY, AI_GAMEPLAY_LIST)
)
async def cb_ai_gameplay(q: CallbackQuery) -> None:
    ...
    list_text, kb = await _game_list_kb(
        _GAMEPLAY_SEL
    )
...
@router.callback_query(
    lambda c: c.data.startswith(_GAMEPLAY_SEL)
)
async def cb_gameplay_select(
    q: CallbackQuery,
    state: FSMContext
) -> None:
    if not await _guard(q):
    ...
    games = await get_all_games(only_active=False)
    game = next(
        (g for g in games if g["slug"] == slug),
        None
    )
    ...
    code = await _load_game_code(
        game["html_file"]
    )

    if code is None:
        await q.answer(
            f"Fayl topilmadi: {game['html_file']}",
            show_alert=True
        )
        return
    ...
    await q.message.edit_text(
        ...
        f"({_fmt_size(len(code.encode()))})\n\n"
        "O'yin mexanikasida nima o'zgartirish kerakligini yozing:\n\n"
        "<i>Misol: «Tezlikni 2 barobarga oshir, "
        "jonlar sonini 5 taga chiqar»</i>",
        reply_markup=_cancel_kb(),
        parse_mode="HTML",
    )
...
@router.message(
    StateFilter(AIGameplayStates.waiting_change)
)
async def msg_gameplay_change(
    m: Message,
    state: FSMContext
) -> None:
    if not await _guard_msg(m):
    ...
    sent = await m.answer(
        f"⏳ <b>Gameplay Designer</b> — "
        f"{data.get('game_name', '')} o'zgartirilmoqda…",
        parse_mode="HTML",
    )

    result = await services.ai_gameplay_change(
        data["game_code"],
        instruction
    )
...
@router.callback_query(
    lambda c: c.data == AI_GAMEPLAY_SAVE_OK
)
async def cb_gameplay_save(
    q: CallbackQuery,
    state: FSMContext
) -> None:
    if not await _guard(q):
...
@router.callback_query(
    lambda c: c.data == AI_GAMEPLAY_DISCARD
)
async def cb_gameplay_discard(
    q: CallbackQuery,
    state: FSMContext
) -> None:
    await state.clear()
...
@router.callback_query(
    lambda c: c.data in (AI_DESIGN, AI_DESIGN_LIST)
)
async def cb_ai_design(q: CallbackQuery) -> None:
    ...
    await q.message.edit_text(
        "🎨 <b>AI UI Designer</b>\n\n"
        "O'yinning vizual qismini "
        "(ranglar, shriftlar, UI elementlar) o'zgartirish.\n\n"
        + list_text,
        reply_markup=kb,
        parse_mode="HTML",
    )
...
@router.callback_query(
    lambda c: c.data.startswith(_DESIGN_SEL)
)
async def cb_design_select(
    q: CallbackQuery,
    state: FSMContext
) -> None:
    if not await _guard(q):
    ...
    if not new_code or not html_file:
        await q.answer(
            "Ma'lumot topilmadi. Qaytadan ko'ring.",
            show_alert=True
        )
        await state.clear()
        return
...
async def cb_ai_design(q: CallbackQuery) -> None:
    ...
    await q.message.edit_text(
        "🎨 <b>AI UI Designer</b>\n\n"
        "O'yinning vizual qismini "
        "(ranglar, shriftlar, UI elementlar) "
        "o'zgartirish.\n\n"
        + list_text,
        reply_markup=kb,
        parse_mode="HTML",
...
@router.message(
    StateFilter(AIDesignStates.waiting_change)
)
async def msg_design_change(
    m: Message,
    state: FSMContext
) -> None:
    if not await _guard_msg(m):
    ...
    sent = await m.answer(
        f"⏳ <b>UI Designer</b> — "
        f"{data.get('game_name', '')} dizayni "
        f"o'zgartirilmoqda…",
        parse_mode="HTML",
    )

    result = await services.ai_design_ui(
        data["game_code"],
        instruction
    )
...
@router.callback_query(
    lambda c: c.data == AI_DESIGN_SAVE_OK
)
async def cb_design_save(
    q: CallbackQuery,
    state: FSMContext
) -> None:
    if not await _guard(q):
...
@router.callback_query(
    lambda c: c.data == AI_DESIGN_DISCARD
)
async def cb_design_discard(
    q: CallbackQuery,
    state: FSMContext
) -> None:
    await state.clear()
...
@router.callback_query(
    lambda c: c.data == AI_IMAGE
)
async def cb_ai_image(
    q: CallbackQuery,
    state: FSMContext
) -> None:
    if not await _guard(q):
    ...
    await q.message.edit_text(
        ...
        "O'yin uchun SVG grafika "
        "(sprite, belgi, fon elementi) yaratish.\n\n"
        "Kerakli rasmni tasvirlab bering:\n\n"
        "<i>Misol: «Kosmik kema — kichik "
        "geometrik, uchburchak asosli korpus, "
        "2 ta qanot, alangali dvigatel»</i>\n\n"
        "<i>Misol: «Oltin tanga — doira, "
        "yaltiroq sariq rang, ichida ¥ belgisi»</i>",
        reply_markup=_cancel_kb(),
        parse_mode="HTML",
    )
...
@router.message(
    StateFilter(AIAssetStates.waiting_description)
)
async def msg_asset_description(
    m: Message,
    state: FSMContext
) -> None:
    if not await _guard_msg(m):
    ...
    result = await services.ai_generate_svg(
        description
    )
    ...
    await m.answer(
        ...
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[[
                InlineKeyboardButton(
                    text="💾 Ha, saqlash",
                    callback_data=AI_IMAGE_SAVE
                ),
                InlineKeyboardButton(
                    text="🗑 Bekor",
                    callback_data=AI_IMAGE_DISCARD
                ),
            ]]
        ),
        parse_mode="HTML",
...
@router.message(
    StateFilter(AIAssetStates.waiting_filename)
)
async def msg_asset_filename(
    m: Message,
    state: FSMContext
) -> None:
    if not await _guard_msg(m):
    ...
    # Strip markdown fences if AI wrapped
    # SVG in ```svg ... ```
    match = re.search(
        r"```(?:svg|xml)?\s*\n?(.*?)\n?```",
        svg_code,
    ...
    try:
        ...
        await m.answer(
            ...
            f"O'lcham: {_fmt_size(size)}\n"
            f"Joylashuv: "
            f"<code>webapp/games/{filename}</code>",
            reply_markup=ai_back_keyboard(),
            parse_mode="HTML",
        )
...
async def cb_image_discard(
    ...
    await q.message.edit_text(
        ...
        "Tekshirilishi kerak bo'lgan HTML5 "
        "kodini yuboring.\n"
        "<i>Butun HTML faylni yoki kod "
        "parchasini yuboring.</i>",
        reply_markup=_cancel_kb(),
        parse_mode="HTML",
    )
...
async def msg_test_code(m: Message, state:
    FSMContext) -> None:
    if not await _guard_msg(m):
        return
...
@router.callback_query(
    lambda c: c.data.startswith(_TEST_SEL)
)
...
@router.callback_query(
    _ALL_P5_STATES,
    lambda c: c.data == AI_CANCEL
)
async def cb_p5_cancel(
    q: CallbackQuery,
    state: FSMContext
) -> None:
...
async def cmd_p5_cancel(
    ...
    await m.answer(
        "✅ Bekor qilindi. /developer "
        "orqali menyuga qaytishingiz mumkin."
    )
    ...
    await q.message.edit_text(
        ...
        f"({_fmt_size(file.size)})\n\n"
        "⚠️ Bu amalni qaytarib bo'lmaydi!",
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[[
                InlineKeyboardButton(
                    text="🗑 Ha, o'chir",
                    callback_data=AI_ASSETS_DEL_OK
                ),
                InlineKeyboardButton(
                    text="❌ Bekor",
                    callback_data=AI_ASSETS_DEL_NO
                ),
            ]]
        ),
        parse_mode="HTML",
...
@router.callback_query(
    lambda c: c.data == AI_ASSETS_UPLOAD
)
async def cb_asset_upload_start(
    q: CallbackQuery,
    state: FSMContext
) -> None:
    if not await _guard(q):
...
@router.message(
    StateFilter(AIAssetManagerFSM.waiting_upload),
    F.document
)
async def msg_asset_upload(
    m: Message,
    state: FSMContext
) -> None:
    if not await _guard_msg(m):
...
async def msg_asset_upload_wrong(
    m: Message
) -> None:
    if not _is_admin(m.from_user.id):
        return
    ...
    await m.answer(
        "⚠️ Faylni <b>document</b> sifatida yuboring.",
        parse_mode="HTML"
    )
...
async def cb_ai_preview(
    ...
    await q.message.edit_text(
        f"👁 <b>Preview</b> "
        f"({len(games)} ta o'yin)\n\n"
        f"Base URL: <code>{webapp_url}</code>\n\n"
        "Tugmani bosib o'yinni to'g'ridan-to'g'ri "
        "oching:\n"
        "(✅ = faol, ❌ = nofaol)",
        reply_markup=InlineKeyboardMarkup(
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🧪 Kodni test qil",
                    callback_data=AI_TEST_CODE
                ),
                InlineKeyboardButton(
                    text="🎮 O'yin faylini test",
                    callback_data=AI_TEST_FILE
                ),
            ],
            [
                InlineKeyboardButton(
...
async def cb_ai_test(
    ...
    await q.message.edit_text(
        ...
        f"Provider: <code>{prov}</code>\n\n"
        "HTML5 o'yin kodini AI yordamida "
...
@router.callback_query(
    lambda c: c.data == AI_TEST_CODE
)
async def cb_test_code_start(
    q: CallbackQuery,
    state: FSMContext
) -> None:
    if not await _guard(q):
...
@router.message(
    StateFilter(AITestStates.waiting_code)
)
async def msg_test_code(
    m: Message,
    state: FSMContext
) -> None:
    if not await _guard_msg(m):
